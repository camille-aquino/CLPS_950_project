...
total_sweep_num=input('')%# of sweeps
file=input('')%abf file name
channel=input('')%4 channels for clampex: Vm_primary (1), Im_sec (2), Vm_prime2 (3), Im_sec2 (4), we choose #3 because we are using a current clamp
samp_frequency=input('')%sampling frequency (20kHz for tested protocol)
data=abfload(file)
storagemat = [];
N_max_spikes = 300;
Threshtrace = [];
...
for a = 1:numsweeps
    ...
    if isempty(above_thres) < 1 %if we have any values above threshold
        ...
        set_cross_minus=  above_thres(find(index_shift_neg));   % find(x) returns nonzero arguments.
